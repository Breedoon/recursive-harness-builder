...
# --- Synthetic error recovery (408 loop detection) ---


def _make_synthetic_error_message():
    """Create a mock message that mimics SDK synthetic API error."""
    msg = MagicMock()
    msg.model = "<synthetic>"
    msg.isApiErrorMessage = True
    msg.content = [TextBlock(text="API Error: 408 Request Timeout")]
    msg.session_id = None
...
def _make_normal_message(text="Hello!"):
    """Create a mock normal assistant message."""
    msg = MagicMock()
...
class TestSyntheticErrorRecovery:
    """Test detection and recovery from SDK synthetic API error loops.

    When the GPT API returns 408s, the SDK retries 10x internally then writes
    a synthetic error message (model="<synthetic>", isApiErrorMessage=True).
    After 2 consecutive synthetic errors, the runner should trigger soft_reset
    + backoff so the next turn gets a fresh SDK process.
    """

    @patch("obs_agent.runner.asyncio.sleep", new_callable=AsyncMock)
    @patch("obs_agent.session.SessionManager.get_client")
    async def test_single_synthetic_error_no_reset(self, mock_get_client, mock_sleep, config):
        """One synthetic error increments counter but does NOT trigger reset."""
        mock_get_client.return_value = _make_mock_client([_make_synthetic_error_message()])
        ...
        assert session_mgr._consecutive_api_errors == 1
        mock_sleep.assert_not_called()

    @patch("obs_agent.runner.asyncio.sleep", new_callable=AsyncMock)
    @patch("obs_agent.session.SessionManager.soft_reset", new_callable=AsyncMock)
    @patch("obs_agent.session.SessionManager.get_client")
    async def test_two_consecutive_synthetic_errors_triggers_reset(
        self, mock_get_client, mock_soft_reset, mock_sleep, config
    ):
        """Two consecutive synthetic errors trigger soft_reset + backoff."""
        mock_get_client.return_value = _make_mock_client([_make_synthetic_error_message()])

        hook_state = HookState()
        from obs_agent.session import SessionManager
        session_mgr = SessionManager(config=config, hook_state=hook_state)
        # Simulate first synthetic error already recorded
        session_mgr._consecutive_api_errors = 1

        runner = ConversationRunner(session_mgr, hook_state, config)
        events = await _collect_events(runner, "hello")

        assert any(isinstance(e, DoneEvent) for e in events)
        assert session_mgr._consecutive_api_errors == 2
        mock_soft_reset.assert_awaited_once()
        mock_sleep.assert_awaited_once_with(30)

    @patch("obs_agent.runner.asyncio.sleep", new_callable=AsyncMock)
    @patch("obs_agent.session.SessionManager.get_client")
    async def test_success_between_synthetic_errors_resets_counter(
        self, mock_get_client, mock_sleep, config
    ):
        """A successful turn between synthetic errors resets the counter."""
        mock_get_client.return_value = _make_mock_client([_make_normal_message()])

        hook_state = HookState()
        from obs_agent.session import SessionManager
        session_mgr = SessionManager(config=config, hook_state=hook_state)
        # Simulate previous synthetic error
        session_mgr._consecutive_api_errors = 1
        ...
        assert any(isinstance(e, DoneEvent) for e in events)
        assert session_mgr._consecutive_api_errors == 0
        mock_sleep.assert_not_called()

    @patch("obs_agent.runner.asyncio.sleep", new_callable=AsyncMock)
    @patch("obs_agent.session.SessionManager.get_client")
    async def test_normal_turns_keep_counter_at_zero(self, mock_get_client, mock_sleep, config):
        """Normal turns don't affect the counter."""
        mock_get_client.return_value = _make_mock_client([_make_normal_message()])
    ...
    @patch("obs_agent.session.SessionManager.soft_reset", new_callable=AsyncMock)
    @patch("obs_agent.session.SessionManager.get_client")
    async def test_recovery_skips_continuation_loop(
        self, mock_get_client, mock_soft_reset, mock_sleep, config
    ):
        """After recovery, continuation loop is skipped (queued messages deferred)."""
        mock_get_client.return_value = _make_mock_client([_make_synthetic_error_message()])

        hook_state = HookState()
        # Queue a message that would trigger continuation
        hook_state.message_queue.put_nowait("queued while stuck")

        from obs_agent.session import SessionManager
        session_mgr = SessionManager(config=config, hook_state=hook_state)
        session_mgr._consecutive_api_errors = 1  # second consecutive

        runner = ConversationRunner(session_mgr, hook_state, config)
        events = await _collect_events(runner, "hello")

        mock_soft_reset.assert_awaited_once()
        # The queued message should be in remaining_pending, not processed as continuation
        assert len(runner.remaining_pending) == 1
        assert runner.remaining_pending[0].text == "queued while stuck"

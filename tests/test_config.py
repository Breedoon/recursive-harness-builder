...
class TestVaultPathResolution:
    ...
    def test_default_vault_path(self):
        cfg = OBSConfig()
        expected = Path.home() / "Documents" / "obs-vault"
...
class TestSessionSettings:
    ...
    def test_current_opus_alias_preserves_effort_and_context(self, alias):
        assert config_module.resolve_model(alias) == "claude-opus-5-5"
        assert config_module.MODEL_EFFORT_LEVELS["claude-opus-5-5"] == "high"
        assert config_module.resolve_model(alias + "[200k]") == "claude-opus-5-5[200k]"

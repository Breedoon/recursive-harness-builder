...
class TestSummarizeToolUse:
    ...
    def test_agent_task_uses_description(self):
        """AgentTask prefers the short user-facing description."""
    ...
    def test_agent_task_truncates_long_prompt(self):
        ...
        long_prompt = "a" * 300
        result = summarize_tool_use("AgentTask", {"prompt": long_prompt})
        assert result.startswith("AgentTask: ")
        assert result.endswith("...")
        assert len(result) == 200
    ...
    def test_agent_task_output_summary(self):
        ...
        assert result == "AgentTaskOutput: agent-123 block=True"

    def test_agent_task_stop_summary(self):
        result = summarize_tool_use("AgentTaskStop", {"task_id": "agent-123"})
        assert result == "AgentTaskStop: agent-123"

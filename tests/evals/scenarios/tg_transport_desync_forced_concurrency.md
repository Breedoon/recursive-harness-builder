...
- Fail if turn 2 (ping) is polluted by leftover turn-1 tool output, or if the final completion summary is emitted before output is actually finished.
...
- The final completion summary should align with true completion, not appear before late backlog from the same logical turn

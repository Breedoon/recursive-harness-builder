...
- Broken behavior example: duplicated/misaligned completion summaries or disjoint turn ordering.
- Suspicious behavior: subtle ordering drift where content is mostly correct but timeline markers are inconsistent.
- Flag odd sequencing (for example duplicate completion summaries, disjointed tool text, or off-order content) even if the strict criteria pass.

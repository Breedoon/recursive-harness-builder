...
class TestCreateOptions:
    ...
    def test_local_qwen_disables_dynamic_tool_loading(self, config):
        mgr = SessionManager(config=config)
        mgr.model_override = "local-qwen3.8-27b[128k]"
        mgr.set_sdk_env_overrides({"ENABLE_TOOL_SEARCH": "true"})
    ...
    def test_other_local_provider_keeps_dynamic_tool_loading_unchanged(self, config):
        mgr = SessionManager(config=config)
        mgr.model_override = "local-gemma4-31b"
    ...
    def test_hosted_provider_keeps_dynamic_tool_loading_unchanged(self, config):
